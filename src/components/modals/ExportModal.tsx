...
export function ExportModal({ project, onClose }: Props) {
  ...
  const filled = (() => {
    const r: Array<{ slot: number; [k: string]: unknown }> = []
    for (let row = 0; row < project.rows; row++)
      for (let c = 0; c < 9; c++) {
        const k = `${row}-${c}`
        const sl = project.slots[k]
        if (sl) r.push({ slot: row * 9 + c, ...sl })
      }
    return r
  })()
  ...
  const gen = (): string => {
    if (tab === 'json') {
      return JSON.stringify({
        menu: { title: project.name, size: project.rows * 9, rows: project.rows },
        ...
        metadata: { exportedAt: new Date().toISOString(), tool: 'MC Menu Designer v4', totalSlots: project.rows * 9, filledSlots: filled.length },
      }, null, 2)
    }
    if (tab === 'yaml') {
