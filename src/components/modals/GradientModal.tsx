...
export function GradientModal({ onClose, onApply }: Props) {
  ...
  return (
    <div className={s.modalOverlay} onClick={onClose}>
      <div className={s.modalDialog} onClick={e => e.stopPropagation()} style={{ minWidth: 520 }}>
        <div className={s.modalTitle}>Градиент-генератор</div>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {GRAD_PRESETS.map((p, i) => (
              <button key={i} className={s.btn} style={{ background: `linear-gradient(90deg,${p.colors.join(',')})`, color: '#fff', textShadow: '0 1px 2px #000', fontSize: 10, padding: '3px 8px' }}
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
            {colors.map((c, i) => (
              <div key={i} style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                ...
                {colors.length > 2 && <button className={s.btn} style={{ padding: '0 4px', fontSize: 10 }} onClick={() => setColors(colors.filter((_, j) => j !== i))}>✕</button>}
              </div>
            ))}
            <button className={s.btn} onClick={() => setColors([...colors, '#FFFFFF'])} style={{ padding: '2px 8px' }}>+</button>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
          <button className={`${s.btn} ${bold ? s.btnPrimary : ''}`} onClick={() => setBold(!bold)} style={{ fontWeight: 700 }}>B</button>
          <button className={`${s.btn} ${italic ? s.btnPrimary : ''}`} onClick={() => setItalic(!italic)} style={{ fontStyle: 'italic' }}>I</button>
        </div>
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <div className={s.modalCode} style={{ flex: 1, fontSize: 11, padding: '8px 10px', maxHeight: 80 }}>{gradTag}</div>
            <button className={s.btn} onClick={() => copy(gradTag, 'mm')}>{copied === 'mm' ? '✓' : 'Коп.'}</button>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <div className={s.modalCode} style={{ flex: 1, fontSize: 11, padding: '8px 10px', maxHeight: 80 }}>{seg2leg(segs)}</div>
            <button className={s.btn} onClick={() => copy(seg2leg(segs), 'leg')}>{copied === 'leg' ? '✓' : 'Коп.'}</button>
          </div>
        </div>

        <div className={s.modalActions}>
          {onApply && <button className={`${s.btn} ${s.btnPrimary}`} onClick={() => { onApply(parseMM(gradTag)); onClose() }}>Применить к имени</button>}
